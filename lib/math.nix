...
{lib, ...}: rec {
  ...
  _pow_int = x: times:
    ...
    then 1 / (_pow_int x (0 - times))
    else multiply (lib.replicate times x);
  ...
  in
    if abs (fractionPart - 0.5) < epsilon
    then
      if intIsEven
      then intPart
      else intPart + 1
  ...
  # Two-argument arctangent with full quadrant handling
  atan2 = y: x:
    if x > 0
    then atan (y / x)
    else if x < 0 && y >= 0
  ...
  in
    if x < 0
    then 1 / (exp (0 - x))
    else (_pow_int e x_int) * (polynomial x_decimal decimal_poly);
  ...
  in
    if x <= 0
    then throw "ln(x<=0) returns invalid value"
    else if x < 1
    then -ln (1 / x)
    else if x > 1.9
    then 2 * (ln (sqrt x))
  ...
  in
    if is_int_times
    then _pow_int x (int times)
    else if x == 0
    then 0
    else if x < 0
    then throw "Calculating power of negative base and decimal exponential is not supported"
    else exp (times * ln x);
  ...
  haversine' = radius: lat1: lon1: lat2: lon2: let
